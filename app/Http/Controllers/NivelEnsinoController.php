...
use App\Http\Requests\NivelEnsinoStoreRequest;
use App\Http\Requests\NivelEnsinoUpdateRequest;
...
class NivelEnsinoController extends Controller
{
    ...
    {
        $registros = $this->nivelensino->all();
    ...
    {
        try {
            $registro = $this->nivelensino->find($id);
    ...
    public function store(NivelEnsinoStoreRequest $request)
    {
        try {
            //Incluindo registro
            $this->nivelensino->create($request->all());
    ...
    public function update(NivelEnsinoUpdateRequest $request, $id)
    {
        try {
            $registro = $this->nivelensino->find($id);
    ...
    public function destroy($id)
    {
        try {
            ...
            if (!$registro) {
                ...
                //Tabela Turmas
                $qtd = DB::table('turmas')->where('nivel_ensino_id', $id)->count();
                ...
                //Tabela Escolas
                $qtd = DB::table('escolas_niveis_ensinos')->where('nivel_ensino_id', $id)->count();
    ...
    {
        $registros = $this->nivelensino->where($field, 'like', '%'.$value.'%')->get();
    ...
    {
        $registros = $this->nivelensino->where($fieldSearch, 'like', '%' . $fieldValue . '%')->get($fieldReturn);
